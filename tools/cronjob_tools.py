...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "deliver": {
                "type": "string",
                "description": "Omit this parameter to auto-deliver back to the current chat and topic (recommended). Auto-detection preserves thread/topic context. Only set explicitly when the user asks to deliver somewhere OTHER than the current conversation. Values: 'origin' (same as omitting), 'local' (no delivery, save only), 'all' (fan out to every connected home channel), platform:chat_id:thread_id for a specific destination, or filesystem:<configured-id> for a trusted target declared under cron.filesystem_delivery_targets in config.yaml. Arbitrary filesystem paths are forbidden. Filesystem targets cannot be mixed with platform targets. Examples: 'telegram:-1001234567890:17585', 'discord:#engineering', 'sms:+155****4567', 'filesystem:ana-live', 'all'. WARNING: 'platform:chat_id' without :thread_id loses topic targeting. 'all' resolves at fire time, so a job created before a channel was wired up will pick it up automatically once connected."
            },
            "skills": {
                "type": "array",
                "items": {"type": "string"},
